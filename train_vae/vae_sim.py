...
class VAE(nn.Module):
    ...
    def encode(self, x, resample=True):
        hidden_states = self.encoder(x)
        cls = hidden_states[:, 0, ...]
        latent = hidden_states[:, 1:, ...]
        assert latent.shape[1] == self.latent_h * self.latent_w, '图像尺寸不正确。'
...
if __name__ == '__main__':
    ...
    # src_path = '/home/dalhxwlyjsuo/criait_tansy/project/EndoViT/our_data/train_clip_data2.json'
    dataset = MedicalCLIPDataset(src_path, transform=transform, base_transform=base_transform, text_key='caption') # disease
    loader  = DataLoader(dataset, batch_size=4, shuffle=True, num_workers=4) # 开启感知为 50，否则为 84.
    encoder_ckpt = '/home/dalhxwlyjsuo/criait_tansy/weight/EndoViT/pytorch_model.bin' # 可有可无。
    ...
    state_dict = torch.load('/home/dalhxwlyjsuo/criait_tansy/project/EndoViT/vae_weight/init_model.bin')
    # state_dict = torch.load('/home/dalhxwlyjsuo/criait_tansy/project/EndoViT/vae_weight/vae_epoch10.pth')
    ...
    # train_clip_vae(vae, loader, device, clip_path='/home/dalhxwlyjsuo/criait_tansy/project/EndoViT/clip_trained_vae_weight_625/CLIPModel_1',
    #                epochs=10, lr=1e-5, beta=.15, commit_beta=.0, train_clip=True, clip_beta1=0.1, clip_cls=.5, perceived_beta= 0.0,
    ...
    #                save_dir='/home/dalhxwlyjsuo/criait_tansy/project/EndoViT/clip_trained_vae_weight_625')

    train_vae(vae, loader, device, epochs=10, lr=1e-5, beta=0.2, commit_beta = 0.5,
              save_dir='/home/dalhxwlyjsuo/criait_tansy/project/EndoViT/vae_weight')
