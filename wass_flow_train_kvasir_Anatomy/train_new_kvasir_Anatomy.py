...
if __name__ == '__main__':
    ...
    json_paths = glob.glob(
        "/mnt/inaisfs/data/home/tansy_criait/new_wass_flow_match/data_tsy1/train_json/data_pairs_flow/*.json")
    ...
    def train_triplet(epochs=20, dataloaders=None):
        ...
        best_acc = evaluate_new_triplet_option(model, dataset, device, generator, 0)
        # best_acc = 0
        ...
        for step in tqdm(range(total_step)):
            ...
            if (step + 1) % save_step == 0:
                ...
                for i in test_json:
                    dataset += json.load(open(i, "r"))
                accuracy = evaluate_new_triplet_option(model, dataset, device, generator, step)
                writer.add_scalar('Evaling/Acc', accuracy, step)
                if best_acc < accuracy:
                    torch.save(model.state_dict(), f"/mnt/inaisfs/data/home/tansy_criait/whole_wass_flow_match/best_flow_weights/attention_tsy.pt")
                    torch.save(classifer.state_dict(), f"/mnt/inaisfs/data/home/tansy_criait/whole_wass_flow_match/best_flow_weights/attention_tsy_classifer.pt")
                    best_acc = accuracy
                else:
                    pass
    ...
    ### 加载 checkpoints
    try:
        state_dict = torch.load("/mnt/inaisfs/data/home/tansy_criait/whole_wass_flow_match/best_flow_weights/attention_tsy.pt", weights_only=True)
        model.load_state_dict(state_dict, strict=True)
        state_dict = torch.load("/mnt/inaisfs/data/home/tansy_criait/whole_wass_flow_match/best_flow_weights/attention_tsy_classifer.pt", weights_only=True)
    ...
    def evaluate_new_triplet_option(model, dataset, device, generator, step=0, k=1):
        ...
        with torch.no_grad():
            for data in tqdm(dataset):
                ...
                wass_A = min(wass_A_list) # 取min或取 sum(wass_A) / len(wass_A), python 无内置 mean。
                ...
                wass_B = min(wass_B_list) # 取min或取 sum(wass_A) / len(wass_A), python 无内置 mean。
                ...
                wass_C = min(wass_C_list) # 取min或取 sum(wass_A) / len(wass_A), python 无内置 mean。
                ...
                wass_D = min(wass_D_list) # 取min或取 sum(wass_A) / len(wass_A), python 无内置 mean。
            ...
            for data in tqdm(dataset):
                ...
                for _ in range(k):
                    ...
                    if "option_D_dir" in data:
                        ...
                        wass_D = [float('inf')]
                wass_A = torch.Tensor([min(wass_A)]) # 取min或取 sum(wass_A) / len(wass_A), python 无内置 mean。
    ...
    train_triplet(epochs=50, dataloaders=dataloaders) 

    dataset = []
    for i in test_json:
        dataset += json.load(open(i, "r"))
    evaluate_triplet(model, dataset, device, generator, 0, 5)
    evaluate_triplet_option(model, dataset, device, generator, 0, 5)
