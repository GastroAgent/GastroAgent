...
def normalize_class_name(name: str) -> str:
    ...
    for standard_name in CLASS_NAMES:
        if standard_name.lower() == name_lower:
...
def extract_flow_probs(flow_item: Dict, num_classes: int = 8) -> np.ndarray:
    """
    从Flow结果中提取概率分布
...
def extract_mllm_probs(mllm_item: Dict, num_classes: int = 8) -> np.ndarray:
    """从MLLM结果中提取概率分布"""
    ...
    if all(f'p_{opt}' in mllm_item for opt in ['A', 'B', 'C', 'D']):
        ...
        if num_classes == 8:
            ...
            # 构造完整的8类概率分布
            probs_8 = np.zeros(num_classes)
            for i, class_name in enumerate(option_classes):
                if class_name in CLASS_TO_IDX:
                    idx = CLASS_TO_IDX[class_name]
                    probs_8[idx] = probs_4[i]

            # 归一化
            if probs_8.sum() > 0:
                probs_8 = probs_8 / probs_8.sum()
            else:
                probs_8 = np.ones(num_classes) / num_classes
...
def fusion_predictions(mllm_item: Dict,
    ...
    # 提取两个模型的概率分布
    p_mllm = extract_mllm_probs(mllm_item, num_classes)
    p_flow = extract_flow_probs(flow_item, num_classes)
