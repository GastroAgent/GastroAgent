...
from flow_matcher_mask import create_generator # 不包含 x0, x1.
...
if __name__ == '__main__':
    ...
    model = TripletNetwork(pretrained=False, freeze_base=False, model='attention', ortho=True)
    # model = TripletNetwork(pretrained=False, freeze_base=False, model='attention', dy=True, ortho=True).to(device)
    ...
    def train_triplet(epochs=20, dataloaders=None):
        ...
        for step in tqdm(range(total_step)):
            ...
            optimizer.zero_grad()
            anchor_emb, hidden_state, ortho_loss1 = model.encode(anchor, False, True)
            negative_emb, ortho_loss2 = model.encode(negative, False, False)
            # with torch.no_grad(): # 待优化。。。
            positive_emb, ortho_loss3 = model.encode(positive, False, False)
            ...
            ) + (ortho_loss1 + ortho_loss2 + ortho_loss3) * 0.1
            ...
            if (step + 1) % save_step == 0:
                ...
                if best_acc < accuracy:
                    torch.save(model.state_dict(), f"/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/best_flow_weights/attention_norm_jac_mask_tsy.pt")
                    torch.save(classifer.state_dict(), f"/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/best_flow_weights/attention_norm_jac_mask_tsy_classifer.pt")
                    best_acc = accuracy
                else:
                    pass
    ...
    ### 加载 checkpoints
    try: # 6347
        state_dict = torch.load("/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/best_flow_weights2/attention_norm_jac_mask_tsy.pt", weights_only=True)
        model.load_state_dict(state_dict, strict=True)
        state_dict = torch.load("/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/best_flow_weights2/attention_norm_jac_mask_tsy_classifer.pt", weights_only=True)
        classifer.load_state_dict(state_dict)
    except:
        state_dict = torch.load("/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/best_flow_weights/attention_norm_jac_tsy.pt", weights_only=True)
        model.load_state_dict(state_dict, strict=True)
        state_dict = torch.load("/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/best_flow_weights/attention_norm_jac_tsy_classifer.pt", weights_only=True)
        classifer.load_state_dict(state_dict)
                ...
                for i in range(len(data['x1_labels'])):
                    ...
                    for _ in range(k):
                        ...
                        with torch.no_grad():
                            ...
                            trajectorys_emb, _ = model.encode(trajectorys, False, False)
    ...
    def evaluate_triplet(model, dataset, device, generator, step=0, k=5):
        ...
        with torch.no_grad():
            for data in tqdm(dataset):
                ...
                anchor_emb, _ = model.encode(anchor)
                ...
                for i in range(k):
                    for idx in range(len(data['x1_labels'])):
                        ...
                        test_emb, _ = model.encode(test_image)
