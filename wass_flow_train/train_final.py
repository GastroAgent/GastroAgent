...
if __name__ == '__main__':
    ...
    def train_triplet(epochs=20, dataloaders=None):
        ...
        for step in tqdm(range(total_step)):
            ...
            if generator is not None:
                ...
                if random.random() < 0.25:
                    for i in range(batch_size):
                        ...
                        batchs_trajectorys.append((trajectorys, 1))
                else:
            ...
            for trajectorys, beta in batchs_trajectorys:
                ...
                for i in range(len(trajectorys)-1):
                    ...
                    ignore = random.sample(list(range(len(trajectorys)-1)), k=min([int(0.5 * (len(trajectorys)-1)), 1]))
                    if i in ignore:
                        wass_loss = cal_wasserstein_loss(last_embeding, cur_embeding).detach()
                    else:
                        wass_loss = cal_wasserstein_loss(last_embeding, cur_embeding)
                    
                    if loss is None:
                        loss = beta * wass_loss if beta > 0 else 1 - torch.clamp(beta * wass_loss, max=0.90)
                    if beta > 0:
                        loss = loss + beta * wass_loss
                    else:
                        loss = loss + 1 - torch.clamp(beta * wass_loss, max=1.0)
            ...
            if (step + 1) % save_step == 0:
                ...
                if best_acc < accuracy:
                    torch.save(model.state_dict(), f"/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/best_flow_weights/sum_attention_dy_tsy.pt")
                    torch.save(classifer.state_dict(), f"/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/best_flow_weights/sum_attention_dy_tsy_classifer.pt")
                    best_acc = accuracy
                else:
                    pass
        if best_acc < accuracy:
