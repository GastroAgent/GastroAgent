...
def smooth_clip_loss(similarity: torch.Tensor, diseases, smooth=0.2, keep_neg=False, true_disease=None) -> torch.Tensor:
    with torch.no_grad():
        ...
        if keep_neg:
            ...
            # 仅对负数部分进行归一化
            normalized_neg_part = torch.clamp(target / denorm_neg_mask, min = - 0.001, max = -1e-6)
        ...
        if true_disease is not None:
            extra_target_smooth = torch.zeros_like(target, dtype=torch.bool)
            for idx_r, disease_r in enumerate(true_disease):
                for idx_l, disease_l in enumerate(true_disease):
                    if idx_l == idx_r:
                        continue
                    extra_target_smooth[idx_r, idx_l] = True if check_match(disease_l, disease_r) else False
            target = torch.where(extra_target_smooth, torch.zeros_like(target), target)
...
def train_clip(loader, device, clip_path = './weight/clip-vit-large-patch14',
    ...
    for epoch in range(1, epochs+1):
        ...
        with (tqdm(loader, desc=f"[Epoch {epoch}/{epochs}]", unit='batch') as pbar):
            for batch in pbar:
                ...
                if gpu_num >= 1:
                    ...
                    logits_per_text = torch.matmul(text_embeds, image_embeds.t().to(text_embeds))

                    logits_per_text = logit_scale * logits_per_text

                    assert int(logits_per_text.shape[0]) == len(texts), "某数据缺少 Text Label。"
                    loss = smooth_clip_loss(logits_per_text, texts, smooth=0.4, keep_neg=True, true_disease=diseases)
                ...
                loss.backward()
                norm = clip_grad_norm_(clip.parameters(), max_norm=5.0).item()
                optimizer_clip.step()
                scheduler_clip.step()
        ...
        if epoch % 2 == 0:
            ...
            if os.path.exists(clip_text_path) and clip_text_path:
                save_clip_text_path = clip_text_path + f'_caption_{epoch}'
...
def train_clip_with_mean_pool(loader, device, clip_path = './weight/clip-vit-large-patch14',
    ...
    for epoch in range(1, epochs+1):
        ...
        if epoch % 1 == 0:
            ...
            if os.path.exists(clip_text_path) and clip_text_path:
                save_clip_text_path = clip_text_path + f'{epoch}'
                clip.text_model.save_pretrained(save_clip_text_path)
