...
if __name__ == '__main__':
    ...
    def train_triplet(epochs=20, dataloaders=None):
        ...
        for step in tqdm(range(total_step)):
            ...
            loss = loss + 0.01 * clip_loss(anchor_emb, anchor_emb) / 3
            loss = loss + 0.01 * clip_loss(positive_emb, positive_emb) / 3
            loss = loss + 0.01 * clip_loss(negative_emb, negative_emb) / 3
            ...
            if (step + 1) % save_step == 0:
                ...
                if best_acc < accuracy:
                    torch.save(model.state_dict(), f"./best_flow_weights_tsy/doctor_exam/attention.pt")
                    torch.save(classifer.state_dict(), f"./best_flow_weights_tsy/doctor_exam/attention_classifer.pt")
                    best_acc = accuracy
                else:
                    pass
    ...
    ### 加载 checkpoints
    try:
        state_dict = torch.load("./best_flow_weights_tsy/doctor_exam/attention.pt", weights_only=True)
        model.load_state_dict(state_dict, strict=True)
        state_dict = torch.load("./best_flow_weights_tsy/doctor_exam/attention_classifer.pt", weights_only=True)
    ...
    model = TripletNetwork(pretrained=False, freeze_base=False, model='attention', dy=True).to(device) # 6946
