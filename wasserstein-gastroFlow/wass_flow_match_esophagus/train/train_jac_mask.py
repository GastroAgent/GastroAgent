...
if __name__ == '__main__':
        ...
        for step in tqdm(range(total_step)):
            ...
            if (step + 1) % save_step == 0:
                ...
                if best_acc < accuracy:
                    torch.save(model.state_dict(), f"./best_flow_weights/attention_norm_jac_mask_tsy.pt")
                    torch.save(classifer.state_dict(), f"./best_flow_weights/attention_norm_jac_mask_tsy_classifer.pt")
                    best_acc = accuracy
                else:
                    pass
    ...
    ### 加载 checkpoints
    try: # 6347
        state_dict = torch.load("./best_flow_weights2/attention_norm_jac_mask_tsy.pt", weights_only=True)
        model.load_state_dict(state_dict, strict=True)
        state_dict = torch.load("./best_flow_weights2/attention_norm_jac_mask_tsy_classifer.pt", weights_only=True)
        classifer.load_state_dict(state_dict)
    except:
        state_dict = torch.load("./best_flow_weights/attention_norm_jac_tsy.pt", weights_only=True)
        model.load_state_dict(state_dict, strict=True)
        state_dict = torch.load("./best_flow_weights/attention_norm_jac_tsy_classifer.pt", weights_only=True)
        classifer.load_state_dict(state_dict)
