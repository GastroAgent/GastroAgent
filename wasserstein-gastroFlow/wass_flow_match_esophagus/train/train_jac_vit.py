...
if __name__ == '__main__':
        ...
        for step in tqdm(range(total_step)):
            ...
            if (step + 1) % save_step == 0:
                ...
                if best_acc < accuracy:
                    torch.save(model.state_dict(), f"./best_flow_weights/attention_norm_jac_tsy_dy_vit.pt")
                    torch.save(classifer.state_dict(), f"./best_flow_weights/attention_norm_jac_tsy_dy_vit_classifer.pt")
                    best_acc = accuracy
                else:
                    pass
    ...
    ### 加载 checkpoints
    try: # dy: 47 non-dy: 49
        state_dict = torch.load("./best_flow_weights2/attention_norm_jac_tsy_vit.pt", weights_only=True)
        model.load_state_dict(state_dict, strict=True)
        state_dict = torch.load("./best_flow_weights2/attention_norm_jac_tsy_vit_classifer.pt", weights_only=True)
