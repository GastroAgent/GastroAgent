...
if __name__ == '__main__':
    ...
    dataloaders.extend(create_dataloaders_by_pairs("./data/食管_Cropped2",
    ...
    def train_triplet(model, dataloaders, criterion, optimizer, device="cuda",
        ...
        best_acc = evaluate_triplet_option(model, dataset, device, generator, 0)
        ...
        for step in tqdm(range(total_step)):
            ...
            if (step + 1) % save_step == 0:
                print("Eval...")
                dataset = json.load(open(test_json, "r"))
                accuracy = evaluate_triplet_option(model, dataset, device, generator, step)
                if best_acc < accuracy:
                    torch.save(model.state_dict(), f"./best_flow_weights/old_model_Disease_Extra.pt")
                    best_acc = accuracy
                else:
                    pass
