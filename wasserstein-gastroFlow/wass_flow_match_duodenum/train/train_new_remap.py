...
from utils_.optim import *

from model_utils.model import *
from model_utils.model import LatentDecoder
from model_utils.my_loss import cal_wasserstein_loss, clip_loss, min_match_wass_loss, TripletLoss, ContrastiveLoss, wasserstein_clip_triplet_loss
...
if __name__ == '__main__':
    ...
    remapper = LatentDecoder(4, 4).to(device)
    ...
    if not use_muon:
        if free_classifer:
            ...
            # 构建参数组
            param_groups = [
                dict(params=hidden_weights, lr=3e-4, weight_decay=0.001),
                dict(params=other_params, lr=3e-4, betas=(0.9, 0.95), weight_decay=0.001),
                {"params": classifer.parameters(), "lr": 1e-4, "weight_decay": 0.001},
                {"params": remapper.parameters(), "lr": 2e-4, "weight_decay": 0.001}
            ]
        ...
        if free_classifer:
            ...
            param_groups = [
                ...
                {"params": remapper.parameters(), "lr": 2e-4, "weight_decay": 0.001, "use_muon":False}
            ]
    ...
    def train_triplet(epochs=20, dataloaders=None):
        ...
        for step in tqdm(range(total_step)):
            ...
            ### 约束损失
            # remapper 
            re_anchor = remapper(anchor_emb.reshape(-1, 4, 16, 16)) * 0.18215 # [B, C * H * W]
            re_positive = remapper(positive_emb.reshape(-1, 4, 16, 16)) * 0.18215
            re_negative = remapper(negative_emb.reshape(-1, 4, 16, 16)) * 0.18215
            loss = 1.5e-1 * (F.mse_loss(re_anchor, anchor) + F.mse_loss(re_positive, positive) + F.mse_loss(re_negative, negative)) / 3
            ...
            ### 三元组损失
            # loss = loss + 0.5 * criterion_triplet(anchor_emb, positive_emb, negative_emb, 1, 1.0)
            ...
            classifer_norm = torch.nn.utils.clip_grad_norm_(classifer.parameters(), 1.0)
            remapper_norm = torch.nn.utils.clip_grad_norm_(remapper.parameters(), 1.0)
            optimizer.step()
            writer.add_scalar('Training/Loss', loss.item(), step)
            writer.add_scalar('Training/Grad Norm', grad_norm.item(), step)
            if step % 10 == 0:
                print(f"Global Step {step}/{total_step} | Loss: {loss.item():.4f} | Grad Norm: {grad_norm.item():.4f} | Classifer Grad Norm: {classifer_norm.item():.4f} | Remapper Grad Norm: {remapper_norm.item():.4f}")
            save_step = 1000
            if (step + 1) % save_step == 0:
                ...
                if best_acc < accuracy:
                    ...
                    torch.save(classifer.state_dict(), f"{resumed_path}/remapper.pt")
                    best_acc = accuracy
                else:
                    pass
    ...
    try:
        ...
        state_dict = torch.load(f"{resumed_path}/remapper.pt", weights_only=True)
        remapper.load_state_dict(state_dict)
    except:
        pass
    ...
    def evaluate_triplet_option(model, dataset, device, generator, step=0, k=1):
        import numpy as np
        if generator is None:
            raise NotImplementedError
        else:
        ...
        with torch.no_grad():
            for data in tqdm(dataset):
                ...
                if not os.path.exists(data["image"]):
                    continue
                answer_id = answer_map[data["gt_letter"]]
                ...
                for _ in range(k):
                    ...
                    caption_templete = "Transform the current unknown medical condition image into an image related to {} condition."
    ...
    def evaluate_triplet(model, classifer, dataset, device, generator, step=0, k=5):
        ...
        with torch.no_grad():
            for data in tqdm(dataset):
                ...
                topk_indices = np.argsort(logits[0].cpu().numpy())[:3]  # lower score is better
   
                # Check whether the ground-truth answer is within the top-k predictions
    ...
    train_triplet(epochs=10, dataloaders=dataloaders) 
