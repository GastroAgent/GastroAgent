...
def get_2d_rotary_pos_embed(
        embed_dim, crops_coords, grid_size, use_real=True, device: Optional[torch.device] = None,
        output_type: str = "np"
):
    """
...
class DoubleImageHintTimeEmbedding(nn.Module):
    def __init__(self, image_embed_dim: int = 768, time_embed_dim: int = 1536, hint_merge=False):
        super().__init__()
        self.image_proj = nn.Linear(image_embed_dim, time_embed_dim)
        self.image_norm = nn.LayerNorm(time_embed_dim)
        self.input_hint_block1 = nn.Sequential(
            nn.Conv2d(3, 16, 3, padding=1),
            nn.SiLU(),
            nn.Conv2d(16, 16, 3, padding=1),
            nn.SiLU(),
        ...
        self.input_hint_block2 = nn.Sequential(
            nn.Conv2d(3, 16, 3, padding=1),
            nn.SiLU(),
            nn.Conv2d(16, 16, 3, padding=1),
            nn.SiLU(),
        ...
        if hint_merge:
            self.hint_merge_block = nn.Sequential(
            ...
            self.hint_merge_block = None

    def forward(self, image_embeds: torch.Tensor, hint1: torch.Tensor, hint2: torch.Tensor, hint_merge=False):
        '''
            image_embeds: [B, D]
            hint1: [B, 3, H, W]. 一般为结构图。
            hint2: [B, 3, H, W]. 一般为色彩图。
        '''
        # image
        time_image_embeds = self.image_proj(image_embeds)
        time_image_embeds = self.image_norm(time_image_embeds)
        hint1 = self.input_hint_block1(hint1)
        hint2 = self.input_hint_block2(hint2)
        if hint_merge and self.hint_merge_block is not None:
            hint = torch.cat([hint1, hint2], dim=1)
            hint = self.hint_merge_block(hint)
            return time_image_embeds, hint
        return time_image_embeds, hint1, hint2
