...
def min_match_mse_loss(anchor_emb, positive_emb):
    ...
    loss = F.mse_loss(anchor_emb, matched_positive, reduction='mean')
    return loss
...
def clip_style_triplet_loss(anchor_emb, positive_emb, negative_emb, temperature=0.07,wass_loss=False):
    """
    ...
    if wass_loss:
        ...
        sim_matrix = torch.zeros([B, B]).to(anchor_emb.device, anchor_emb.dtype)
    else:
