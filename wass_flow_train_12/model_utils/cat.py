...
class AveragedKeyCircularConvolutionalAttention(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, context: Optional[torch.Tensor] = None, *args, **kwargs
    ) -> torch.Tensor:
        """
        Args:
...
class GatedMLP(nn.Module):
    """Gated MLP as used in Qwen / LLaMA / PaLM: SwiGLU activation."""
    def __init__(
        self,
        in_features: int,
        ...
        # Note: In Qwen, the intermediate size is often larger (e.g., 27392 for d_model=4096)
        # But here we follow general practice: up_proj and gate_proj map to hidden_features
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        gate = F.silu(self.gate_proj(x))      # (B, N, H)
        up = self.up_proj(x)                  # (B, N, H)
        fused = gate * up                     # (B, N, H)
        out = self.down_proj(fused)           # (B, N, C)
        return self.dropout(out)


class CATEncoderLayer(nn.Module):
    r"""TransformerEncoderLayer with custom attention + Qwen-style GatedMLP."""
    ...
    def __init__(
        ...
        attn_drop: float = 0.0,
        proj_drop: float = 0.0,
        mlp_dropout: float = 0.0,  # separate dropout for MLP if desired
    ):
        super().__init__()
        assert batch_first, "This implementation assumes batch_first=True"

        self.norm_first = norm_first

        # Custom attention
        self.self_attn = AveragedKeyCircularConvolutionalAttention(
            dim=d_model,
            num_heads=nhead,
            qkv_bias=qkv_bias,
            attn_drop=attn_drop,
        ...
        # Replace standard MLP with GatedMLP (SwiGLU)
        self.mlp = GatedMLP(
            in_features=d_model,
        ...
        # Note: MLP already has its own dropout; you may remove this if redundant
    ...
    def forward(
        ...
        **kwargs,
        ...
        # src_mask and src_key_padding_mask are ignored (as before)
