...
def sinkhorn_loss(bx, bx1, epsilon=0.1, n_iter=50, reduction='mean', **kwargs):
    ...
    if bx.ndim == 2:
        B, D = bx.shape
        H = W = int((D // 4) ** 0.5)
        bx = bx.view(B, 4, H, W).permute(0, 2, 3, 1).reshape(B, H*W, 4)
    elif bx.ndim == 4:
        B, C, H, W = bx.shape
        bx = bx.permute(0, 2, 3, 1).reshape(B, H*W, C)
    elif bx.ndim == 3:
    ...
    if bx1.ndim == 2:
        B, D = bx1.shape
        H = W = int((D // 4) ** 0.5)
        bx1 = bx1.view(B, 4, H, W).permute(0, 2, 3, 1).reshape(B, H*W, 4)
    elif bx1.ndim == 4:
        B, C, H, W = bx1.shape
        bx1 = bx1.permute(0, 2, 3, 1).reshape(B, H*W, C)
    elif bx1.ndim == 3:
...
def clip_style_triplet_loss(anchor_emb, positive_emb, negative_emb, temperature=1.67):
    """
...
def kl_divergence(mu_q, logvar_q, mu_p, logvar_p, eps=1e-8):
    """
    Compute KL(N(q) || N(p)) for diagonal Gaussians.
    All inputs: (B, C, H, W)
