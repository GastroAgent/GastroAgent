...
if __name__ == '__main__':
    ...
    def train_triplet(epochs=20, dataloaders=None):
        ...
        for step in tqdm(range(total_step)):
            ...
            anchor, positive, negative = anchor.to(device), positive.to(device), negative.to(device) # 已乘以 0.18215
            optimizer.zero_grad()
            ...
            neg_mse_loss = 1 - torch.clamp_max(min_match_mse_loss(negative_emb, positive_emb), 1.0)
    ...
    def evaluate_triplet(model, dataset, device, generator, step=0, k=10):
        if generator is None:
            raise NotImplementedError
        else:
            vae = generator.vae
