...
def parse_arguments():
    ...
    parser.add_argument("--output_dir", type=str,
                        default="/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/train/train_gan_v2/base-flow-match_vae",
...
def train(args):
    ...
    globe_dataset = MedicalJsonDataset(
        path=json_path,
        transform=transform,
        hint_transform=transform_grey,
        transform_A=transform_A,
    ...
    print('DataLoad Total Steps: ', sum([len(dataloader) for dataloader in dataloaders]) + len(globe_dataloader))
    dataloopers = [infiniteloop(dataloader) for dataloader in dataloaders] 
    globe_datalooper = infiniteloop(globe_dataloader)
    ...
    try:
        ...
        print(e)
        pass
    # vae = AutoencoderKL.from_pretrained('/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/flow_matcher_otcfm/vae_our').eval()
    ...
    if use_image_feature:
        ...
        vision_model.device = text_model.device
    else:
        vision_model = None
    ...
    if latest_model:
        ...
        start_step = checkpoint['step'] - 10000
    ...
    with trange(start_step, args.total_steps, initial=start_step, total=args.total_steps,
        ...
        for step in step_pbar:
            ...
            if random.random() < 0.05:
                ...
                datalooper = globe_datalooper
            batch = next(datalooper)
            batch_size = args.batch_size

            x0 = batch['x0']
            x1 = batch['x1'] + torch.randn_like(batch['x1']) * 0.012
            x1_path = batch['x1_path']
            caption = batch['caption']
            hint = batch['hint'] + torch.randn_like(batch['hint']) * 0.012
            ...
            vision_embeds = vision_embeds + torch.randn_like(vision_embeds) * 0.0
