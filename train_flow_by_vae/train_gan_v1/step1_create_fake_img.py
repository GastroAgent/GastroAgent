...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/train/train_gan_v2/fake_eval/fake_60000_vit-vae_few-step',
    ...
    def generate(self):
        ...
        for dataset in datasets:
            for idx in tqdm(range(0, len(dataset), batch_size)):
                ...
                if idx > 200:
                    break
                batch1 = deepcopy(dataset[idx])
                dataset2 = random.choice(datasets)
                batch2_idx = random.choice(range(len(dataset2)))
                batch2 = dataset2[batch2_idx]
                batch = deepcopy(batch2)
                batch['x0'] = batch1['x0']
                batch['label_A'] = batch1['label_A']
                batch['label_A_id'] = batch1['label_A_id']
                batch['x0_path'] = batch1['x0_path']
                if idx % 5 == 0:
                    try:
