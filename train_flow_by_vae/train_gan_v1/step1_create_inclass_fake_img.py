...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str,
                        default = '/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/train/train_gan_v2/base-flow-match_vae/otcfm/otcfm_weights_step_60000.pt',
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/train/train_gan_v2/hard_fake_eval/hard_fake_60000_vit-vae',
...
class ImageGenerator:
    ...
    def _load_model(self, checkpoint: str, use_ema=False):
        ...
        if not self.use_gt_vt:
            ...
            vae = VAE(latent_dim=4, encoder_ckpt=encoder_ckpt, decoder_ckpt=decoder_ckpt, use_VQVAE=False).eval().to('cuda:0')
            # Optional: load pre-trained VAE
            vae.device = 'cuda:0'
            state_dict = torch.load('/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/train/train_gan_v2/vit-vae/vit_vae_ema.pth', map_location='cuda:0')
            ...
            # Optional: load pre-trained VAE
            vae.device = device
            state_dict = torch.load('/mnt/inaisfs/data/home/tansy_criait/wass_flow_match_tsy/train/train_gan_v2/vit-vae/vit_vae_ema.pth', map_location=device)
            vae.load_state_dict(state_dict, strict=False)
            print("Loaded pre-trained VAE weights.")
        ...
        for dataset in datasets:
            for idx in tqdm(range(0, len(dataset), batch_size)):
                ...
                t, xt, ut = FM.get_sample_location_and_conditional_flow(x0, x1,
                                                                        ...
                                                                        cond=conds,
                                                                        print_info=False)
                # self.args.num_steps = random.choice([8, 9, 10, 11, 12])
                self.args.num_steps = random.choice([8, 6, 4, 2])
                ...
                image_dir = os.path.join(output_dir, f"{label_A}###{label_B}")
                ...
                for x in trajectory[::2]:
                    x = self.vae.decode(x / 0.18215).sample
                    sample = self.normalize_samples(x)
                    img_path = os.path.join(image_dir, f'step{step}_{x0_name2}#to#{x1_name}')
