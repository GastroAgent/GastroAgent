...
class Qwen2VLModule(VLMBaseModule):
    ...
    def get_model_class(self, model_id: str, model_init_kwargs: dict):
        if "Qwen2-VL".lower() in model_id.lower():
            model_cls = Qwen2VLForConditionalGeneration
        elif "Qwen2.5-VL".lower() in model_id.lower():
    ...
    def prepare_model_inputs(self, processing_class, prompts_text, images, return_tensors="pt", padding=True,
        ...
        prompts_text = [x.replace('<|Image_start|><image><|Image_end|>',
                                  f'<|vision_start|>{processing_class.image_token}<|vision_end|>') for x in
                        prompts_text]

        images = [[img.resize((336, 336), Image.Resampling.LANCZOS)] for img in images]
        if len(images) > 0:
