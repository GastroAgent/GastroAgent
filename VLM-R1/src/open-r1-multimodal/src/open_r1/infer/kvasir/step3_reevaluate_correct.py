...
# ===== 配置参数 =====
data_name = 'kvasir'
input_data_path = f'/mnt/inaisfs/data/home/tansy_criait/VLM-R1/data/Eval/kvasir_doctor_exam/cot-419-0.4/new_eval_tsy_llm_extracted.json'
output_data_path = f'/mnt/inaisfs/data/home/tansy_criait/VLM-R1/data/Eval/kvasir_doctor_exam/cot-419-0.4/new_eval_tsy_llm_final.json'
...
def get_gt_letter(item: dict) -> str:
    """
    从item中提取ground truth的选项字母，增加同义/模糊匹配逻辑
    """
    gt_text = item.get("gt_answer", None)
    if gt_text is None:
        return None

    # 归一化处理函数：去除空格、下划线并转小写
    def normalize_text(text):
        if not isinstance(text, str):
            text = str(text)
        return text.replace("_", "").replace(" ", "").lower()

    target_norm = normalize_text(gt_text)
    ...
    for letter in "ABCDEFGH":
        ...
        if key in item and item[key] is not None:
            ...
            # 对选项值也进行同样的归一化处理
            if normalize_text(option_value) == target_norm:
                return letter
