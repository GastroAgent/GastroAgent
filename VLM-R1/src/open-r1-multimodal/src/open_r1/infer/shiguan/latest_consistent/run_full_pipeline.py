"""
================================================================================
完整Pipeline执行脚本 (自动执行版)
================================================================================
功能: 按顺序自动执行所有4个步骤，无需人工确认
...
STEPS = [
    {
        ...
        'script': '/mnt/inaisfs/data/home/tansy_criait/VLM-R1/src/open-r1-multimodal/src/open_r1/infer/shiguan/latest_consistent/step1_model_inference.py',
        'description': '对输入数据进行模型推理，计算概率分布和trigger指标',
    },
    {
        'name': '步骤2: LLM提取答案',
        'script': '/mnt/inaisfs/data/home/tansy_criait/VLM-R1/src/open-r1-multimodal/src/open_r1/infer/shiguan/latest_consistent/step2_extract_answers_with_llm_latest.py',
        'description': '使用LLM从生成文本中提取标准化答案',
    },
    {
        'name': '步骤3: 重新评估correct',
        'script': '/mnt/inaisfs/data/home/tansy_criait/VLM-R1/src/open-r1-multimodal/src/open_r1/infer/shiguan/latest_consistent/step3_reevaluate_correct.py',
        'description': '根据提取的答案重新计算correct字段',
    },
    {
        'name': '步骤4: 重新分析Trigger性能',
        'script': '/mnt/inaisfs/data/home/tansy_criait/VLM-R1/src/open-r1-multimodal/src/open_r1/infer/shiguan/latest_consistent/step4_reanalyze_trigger_performance.py',
...
def run_step(step_info: dict, step_num: int) -> bool:
    ...
    try:
        # 传递当前环境变量，确保LD_LIBRARY_PATH等设置生效
        env = os.environ.copy()
        
        result = subprocess.run(
            [sys.executable, script_path],
            check=True,
            capture_output=False,  # 实时显示输出到控制台
            text=True,
            env=env
        )
...
def main():
    ...
    """
    print("=" * 80)
    print("医疗VQA推理与分析完整Pipeline (自动执行模式)")
    print("=" * 80)
    print("\n流程概览:")
