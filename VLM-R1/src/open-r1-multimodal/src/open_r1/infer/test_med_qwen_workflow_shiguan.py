...
【脚本概述】
本脚本用于医疗领域的视觉问答（VQA）任务推理，基于Qwen-VL多模态大模型
...
1. 模型初始化与加载
   - 加载预训练的Qwen-VL模型（医疗领域微调版本）
   - 支持LoRA权重加载和合并
   - 配置Flash Attention 2加速推理
   - 初始化图像和文本处理器（AutoProcessor）
...
【文件路径配置】
- 模型路径: /mnt/inaisfs/data/home/tansy_criait/weights/{model_name} (Qwen-VL模型)
...
# 保留对workflow类的引用，用于概率计算（如果Qwen-VL模型有相应方法）
try:
    from llava.model.language_model.llava_qwen_mul_workflow import PloyLlavaLlamaForCausalLM
except:
    PloyLlavaLlamaForCausalLM = None

# ===== 配置参数 =====
# Qwen-VL模型路径，可根据需要修改
model_name = 'Qwen3-VL-8B-Instruct'  # 或 'Med-Qwen2.5-VL-7B-UnbiasGRPO' 等
model_id = f'/mnt/inaisfs/data/home/tansy_criait/weights/{model_name}'
data_name = '食管'
batch_size = 1

data_path = f'/mnt/inaisfs/data/home/tansy_criait/VLM-R1/data/Eval/doctor_食管/new_eval_tsy_llm.json'
lora_model_path = None
model_save_path = None

data_save_path = f'/mnt/inaisfs/data/home/tansy_criait/VLM-R1/results/doctor_食管_workflow_gemini/{model_name}/{data_name}.json'
os.makedirs(f'/mnt/inaisfs/data/home/tansy_criait/VLM-R1/results/doctor_食管_workflow_gemini/{model_name}', exist_ok=True)
...
model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
    ...
    max_memory={0: "40GiB"} if torch.cuda.is_available() else None,
)
...
for ch in choice_tokens:
    ...
    choice_ids.append(ids[0] if len(ids) > 0 else None)
choice_ids = [cid for cid in choice_ids if cid is not None]
choice_ids = torch.tensor(choice_ids, device=model.device) if len(choice_ids) > 0 else None
...
generation_config = GenerationConfig(
    ...
    do_sample=True,
    temperature=0.2,
    pad_token_id=processing_class.tokenizer.pad_token_id if hasattr(processing_class.tokenizer, 'pad_token_id') else None,
)
...
for i in range(0, len(dataset), batch_size):
    ...
    for x in inputs:
        # 获取图像路径
        image_files = get_key_from_inputs(x, "image_paths") if "image_paths" in x and x["image_paths"] is not None else []
        
        # 构建Qwen-VL格式的消息
        images_content = [{"type": "image", "image": img_path} for img_path in image_files]
        
        # 获取用户文本（移除Llava格式的Image标签）
        user_text = x.get('formatted_text', '').replace('Image: <|Image_start|><image><|Image_end|>', '').strip()
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": images_content + [{"type": "text", "text": user_text}],
            }
        ]
    ...
    # 编码输入（Qwen-VL格式）
    # 注意：Qwen-VL的processor处理方式与Llava不同
    if len(image_inputs_list) > 0 and any(len(imgs) > 0 for imgs in image_inputs_list):
        # 有图像的情况
        # 需要为每个样本单独处理，因为图像数量可能不同
        prompt_inputs_list = []
        for text, image_inputs, video_inputs in zip(texts, image_inputs_list, video_inputs_list):
            ...
            prompt_inputs_list.append(inputs)
        # 对于batch_size=1的情况，直接使用第一个
        prompt_inputs = prompt_inputs_list[0] if len(prompt_inputs_list) > 0 else None
    else:
        # 无图像的情况
    ...
    # ===== 计算概率分布和trigger策略 =====
    with torch.no_grad():
        # Qwen-VL可能没有get_pG_from_inputs方法，需要手动计算概率分布
        if hasattr(model, 'get_pG_from_inputs') and choice_ids is not None:
            # 如果模型有这个方法，直接使用
            ...
            # 计算选项token的概率
            if choice_ids is not None and len(choice_ids) > 0:
                # 提取选项token的logits
                choice_logits = last_token_logits[:, choice_ids]  # [batch_size, num_choices]
                # 计算softmax概率
                p_G = F.softmax(choice_logits, dim=-1)  # [batch_size, num_choices]
            else:
                # 如果没有choice_ids，创建一个全零的概率分布
                batch_size = last_token_logits.shape[0]
                p_G = torch.zeros(batch_size, len(choice_tokens), device=last_token_logits.device)
                print("[WARN] 无法计算选项概率，使用零分布")
        ...
        # 如果模型有trigger_policy_from_pG方法，使用它；否则手动计算
        if hasattr(model, 'trigger_policy_from_pG'):
            ...
            # 手动计算trigger policy
            # 计算熵
            entropy = -torch.sum(p_G * torch.log(p_G + 1e-10), dim=-1)  # [batch_size]
            # 计算最大概率和argmax
            max_prob, argmax_idx = p_G.max(dim=-1)  # [batch_size]
            # 计算high_entropy和low_confidence
            high_entropy = entropy > theta_u
            low_confidence = max_prob < theta_p
            # 构建policy字典
            policy = {
                "trigger_mask": high_entropy | low_confidence,
                "high_entropy": high_entropy,
                "low_confidence": low_confidence,
                "ood_flag": torch.zeros_like(high_entropy, dtype=torch.bool),
                "rule_flag": is_tail_qtype,
                "entropy": entropy,
                "max_prob": max_prob,
    ...
    generated_ids_trimmed = [
        out_ids[len(in_ids):] for in_ids, out_ids in zip(prompt_inputs.input_ids, generate_returned_result)
    ]
    ...
    # ===== 处理每个样本的结果 =====
    for b_idx, (generated_text, input_) in enumerate(zip(completions, inputs)):
        # 获取对应的prompt text
        prompt_text = texts[b_idx] if b_idx < len(texts) else input_.get('formatted_text', '')
