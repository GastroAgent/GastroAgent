...
def _mask_targets(target, tokenized_lens, speakers):
    # cur_idx = 0
    cur_idx = tokenized_lens[0] # 系统提示词
    tokenized_lens = tokenized_lens[1:]
    target[:cur_idx] = IGNORE_INDEX
    for tokenized_len, speaker in zip(tokenized_lens, speakers):
        if speaker == "human": # '### Human': [14374,  11097]
        ...
        for sentence in source:
            if DEFAULT_IMAGE_TOKEN in str(sentence['value']):
                ...
                if "mmtag" in conversation_lib.default_conversation.version:
                    sentence['value'] = sentence['value'].replace(DEFAULT_IMAGE_TOKEN, '<|image_start|>' + DEFAULT_IMAGE_TOKEN + '<|image_end|>')
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:
            ...
            if isinstance(images[0], list):
                ...
                if len(valid_imgs) > 0:
                    # 只有在有非空 img 才去堆叠
                    images = [torch.stack(img, dim=0) if not img.shape[0] == 0 else img for img in valid_imgs]
                else:
                    # 全部都是空的话，这里你可以选择跳过本次循环
                    # 比如放到一个 DataLoader 的 for-loop 里，就写 continue
                    # 或者把 images 设成一个空 tensor，根据后续逻辑定
                    pass
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.tune_mm_mlp_adapter: # 只 finetune mlp。
