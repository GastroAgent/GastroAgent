...
with open(os.path.expanduser(args.text_data_path), "r") as f:
    questions = json.load(f)  # 加载整个 JSON 文件，而不是逐行解析
    print(f"[DEBUG] Loaded {len(questions)} questions from {args.text_data_path}")
...
for idx in tqdm(range(args.eval_num)):
    ...
    with torch.inference_mode():
        outputs = model.generate(
            ...
            images=image_tensors.to(dtype=torch.float16, device='cuda', non_blocking=True),
