...
def eval_model(args, question_file, answers_base_path):
    ...
    if args.no_tag:
        if "-zh-" not in answers_base_path:
            pred_dict, correct_precentage = MedicalEvalNoTag(questions, res, tag=True)
        else:
            pred_dict, correct_precentage = MedicalEvalNoTagZh(questions, res, tag=True)
    else:
...
def MedicalEvalNoTag(data_dict:list, pred_dict: list, **kwargs) -> tuple:
    ...
    succ = 0
    gt_answer_id = {'A':0, 'B':1, 'C':2, 'D':3}
    new_options = {x['question_id']: x for x in data_dict}
    wrong_dataset = []
    # print('pred_dict',pred_dict)
    for idx, (_, data) in enumerate(zip(data_dict, pred_dict)):
        options = new_options[data['question_id']]
        ...
        try:
            ...
            pred_choice = extract_choice(target_str)
            gt_answer = extract_choice(options['answer'])
            ...
            most_similar_index = find_most_similar_index(answer_list, target_str)
            if answer_list[most_similar_index] == data['gt_answer'] or answer_list[most_similar_index] == answer_list[gt_answer_id[gt_answer]]:
                succ += 1
                data['is_correct'] = 'yes'
                continue
            else:
    ...
    # with open('/home/work/GRPO_result/Llava-Qwen2-32B-tune-med/wrong_dataset.json', 'w') as f:
    #     json.dump(wrong_dataset, f, indent=4, ensure_ascii=False)
...
def MedicalEvalNoTagZh(data_dict:list, pred_dict: list, **kwargs) -> tuple:
    ...
    for idx, (_, data) in enumerate(zip(data_dict, pred_dict)):
        ...
        try:
            ...
            # target_str = data['extracted_answer']
            pred_choice = extract_choice_zh(target_str)
            gt_answer = extract_choice_zh(options['answer'])
            ...
            continue
    with open('/home/work/GRPO_result/Llava-Qwen2-32B-tune-med/zh_wrong_dataset.json', 'w') as f:
        json.dump(wrong_dataset, f, indent=4, ensure_ascii=False)
    return pred_dict, succ / tot
...
if __name__ == "__main__":
    ...
    parser.add_argument('--answers_base_path', type=str, 
                        default="/mnt/inaisfs/data/home/tansy_criait/VLM-R1/results/Kvasir/Llava-Qwen2-7B-tune-med-v0-mmtag-mul-Latest-75/Kvasir-en.json")
