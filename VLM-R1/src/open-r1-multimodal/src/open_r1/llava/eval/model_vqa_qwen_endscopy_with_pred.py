...
sys.path.append('/home/dalhxwlyjsuo/criait_tansy/jmf/VLM-R1/src/open-r1-multimodal/src/open_r1') # 服务器执行
import difflib
...
def eval_model(args, question_file, answers_base_path):
    ...
    if args.no_tag:
        if "Kvasir-zh" in answers_base_path:
            pred_dict, correct_precentage = MedicalEvalNoTagZh(questions, res)
        else:
            pred_dict, correct_precentage = MedicalEvalNoTag(questions, res)
    else:
        if "Kvasir-zh" in answers_base_path:
    ...
    answers_base_path = answers_base_path.replace('.json', '') + f'-{args.no_tag}' + f'-{correct_precentage:.3f}.json'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--question-file", type=str, default="/home/work/ds-vl-on-policy-2/Kvasir-en.json")
    ...
    parser.add_argument('--answers_base_path', type=str, default="/home/work/GRPO_result/LlavaQwen2-GRPO-Tricks-Total/Kvasir-en-False.json")
