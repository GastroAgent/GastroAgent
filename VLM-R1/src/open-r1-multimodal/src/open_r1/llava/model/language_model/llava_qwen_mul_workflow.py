...
class PloyLlavaLlamaForCausalLM(Qwen2ForCausalLM, LlavaMetaForCausalLM):
    ...
    # ======================= GastroAgent diagnostic utilities =======================

    @staticmethod
    def get_pG_from_logits(
        logits: torch.Tensor,
        candidate_ids: Optional[Union[torch.LongTensor, List[int]]] = None,
        take_last_step: bool = True,
    ) -> torch.Tensor:
        """
        将语言模型 logits 转成诊断概率分布 p_G(c|q).
        logits: [B, V] 或 [B, T, V]
        """
        if logits.dim() == 3 and take_last_step:
            logits = logits[:, -1, :]   # [B, V]

        if candidate_ids is not None:
            if not torch.is_tensor(candidate_ids):
                candidate_ids = torch.tensor(candidate_ids, device=logits.device, dtype=torch.long)
            logits = logits.index_select(dim=-1, index=candidate_ids)  # [B, C]

        p_G = F.softmax(logits, dim=-1)
        return p_G

    @staticmethod
    def compute_pg_stats(
        p_G: torch.Tensor,
        tail_indices: Optional[Iterable[int]] = None,
        eps: float = 1e-12,
    ) -> Dict[str, torch.Tensor]:
        """
        根据 p_G(c|q) 计算熵 / 最大概率 / argmax / 是否 tail 类
        """
        p_G = p_G.clamp_min(eps)
        p_G = p_G / p_G.sum(dim=-1, keepdim=True)

        entropy = -(p_G * p_G.log()).sum(dim=-1)        # [B]
        max_prob, argmax_idx = p_G.max(dim=-1)          # [B], [B]

        if tail_indices is None:
            is_tail_pred = torch.zeros_like(argmax_idx, dtype=torch.bool, device=argmax_idx.device)
        else:
            C = p_G.size(-1)
            tail_mask = torch.zeros(C, dtype=torch.bool, device=p_G.device)
            tail_idx_tensor = torch.tensor(list(tail_indices), dtype=torch.long, device=p_G.device)
            tail_mask[tail_idx_tensor] = True           # [C]
            is_tail_pred = tail_mask[argmax_idx]        # [B]
    ...
    @staticmethod
    def trigger_policy_from_pG(
        p_G: torch.Tensor,
        tail_indices: Iterable[int],
        theta_u: float,
        theta_p: float,
        ood_distance: Optional[torch.Tensor] = None,
        theta_ood: Optional[float] = None,
        rule_based_flag: Optional[torch.Tensor] = None,
        eps: float = 1e-12,
    ) -> Dict[str, Any]:
        """
        Trigger policy:
          - H_G(q) > theta_u
          - max_c p_G(c|q) < theta_p
          - argmax_c p_G(c|q) ∈ T
          - OOD 距离 > theta_ood
          - rule_based_flag=True
        """
        stats = PloyLlavaLlamaForCausalLM.compute_pg_stats(p_G, tail_indices=tail_indices, eps=eps)
        entropy     = stats["entropy"]       # [B]
        max_prob    = stats["max_prob"]      # [B]
        is_tail_pred = stats["is_tail_pred"] # [B]

        high_entropy   = entropy > theta_u
        low_confidence = max_prob < theta_p
        tail_pred      = is_tail_pred

        if ood_distance is not None and theta_ood is not None:
            ood_flag = ood_distance > theta_ood
        else:
            ood_flag = torch.zeros_like(high_entropy, dtype=torch.bool, device=high_entropy.device)

        if rule_based_flag is None:
            rule_based_flag = torch.zeros_like(high_entropy, dtype=torch.bool, device=high_entropy.device)

        trigger_mask = high_entropy | low_confidence | tail_pred | ood_flag | rule_based_flag
        ...
        return {
            ...
            "tail_pred":      tail_pred,
            "ood_flag":       ood_flag,
            "rule_flag":      rule_based_flag,
            "entropy":        entropy,
            "max_prob":       max_prob,
            "argmax_idx":     stats["argmax_idx"],
            "is_tail_pred":   is_tail_pred,
        }

    @torch.no_grad()
    def get_pG_from_inputs(
        self,
    ...
    @torch.no_grad()
    def trigger_policy_from_inputs(
        self,
        input_ids: torch.LongTensor,
        attention_mask: Optional[torch.Tensor],
        images: Optional[torch.FloatTensor],
        image_sizes: Optional[List[List[int]]],
        candidate_ids: Union[torch.LongTensor, List[int]],
        tail_indices: Iterable[int],
        theta_u: float,
        theta_p: float,
        ood_distance: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        """
        直接对应 Trigger policy：
          内部先算 p_G(c|q)，再根据阈值判断是否触发 WG-Flow
        """
        p_G, outputs = self.get_pG_from_inputs(
            input_ids=input_ids,
            attention_mask=attention_mask,
            images=images,
            image_sizes=image_sizes,
        ...
        policy = self.trigger_policy_from_pG(
            p_G=p_G,
            ...
            theta_ood=theta_ood,
            rule_based_flag=rule_based_flag,
        )
        policy["p_G"] = p_G
